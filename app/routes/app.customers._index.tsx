    ...
    <s-page heading="Customer Points">
      ...
      <s-section>
            ...
            <fetcher.Form method="post">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
                <div>
                  ...
                  <input name="customerEmail" type="email" placeholder="customer@email.com" required className="lp-input" style={{ width: "100%", padding: "8px 10px" }} />
                </div>
                <div>
                  <label style={{ fontSize: "12px", fontWeight: "600", color: "#374151", display: "block", marginBottom: "4px" }}>Full Name</label>
                  <input name="customerName" type="text" placeholder="Afan Aleem" className="lp-input" style={{ width: "100%", padding: "8px 10px" }} />
                </div>
                <div>
                  <label style={{ fontSize: "12px", fontWeight: "600", color: "#374151", display: "block", marginBottom: "4px" }}>
                    Shopify Customer ID <span style={{ color: "#9ca3af", fontWeight: "400" }}>(optional)</span>
                  </label>
                  <input name="shopifyId" type="text" placeholder="e.g. 9242473529602" className="lp-input" style={{ width: "100%", padding: "8px 10px" }} />
                  <div style={{ fontSize: "11px", color: "#6b7280", marginTop: "3px" }}>In Shopify Admin → Customers → click customer → copy ID from URL</div>
                </div>
                <div>
                  <label style={{ fontSize: "12px", fontWeight: "600", color: "#374151", display: "block", marginBottom: "4px" }}>Initial Points</label>
                  <input name="initialPoints" type="number" min="0" placeholder="0" className="lp-input" style={{ width: "100%", padding: "8px 10px" }} />
                </div>
              </div>
              <button type="submit" disabled={isSubmitting} className="lp-btn lp-btn-primary" style={{ padding: "10px 24px" }}>
        ...
        {customers.length === 0 ? (
          <EmptyState
            ...
            subtitle={search ? `No results for "${search}". Try a different search.` : "Click \"+ Enroll Customer\" to add customers manually, or points are awarded automatically when orders are paid."}
                    ...
                    <tr key={c.customerId}>
                      ...
                      <td>
                        <fetcher.Form method="post" style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                          ...
                          <input name="pointsDelta" type="number" placeholder="+/−" className="lp-input" style={{ width: "72px", padding: "6px 8px" }} required />
                          <input name="note" type="text" placeholder="Reason" className="lp-input" style={{ width: "120px", padding: "6px 8px" }} />
                          <button type="submit" className="lp-btn lp-btn-primary lp-btn-sm">Apply</button>
                        </fetcher.Form>
                      </td>
                      <td>
                        <Link to={`/app/customers/${encodeURIComponent(c.customerId)}`} className="lp-btn lp-btn-secondary lp-btn-sm" style={{ textDecoration: "none", marginRight: "6px" }}>View</Link>
                        <Link to={`/app/transactions?customerId=${encodeURIComponent(c.customerId)}`} className="lp-btn lp-btn-secondary lp-btn-sm" style={{ textDecoration: "none" }}>History</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <s-section heading="Why aren't all my Shopify customers here?">
        <div style={{ fontSize: "13px", color: "#374151", lineHeight: "1.8" }}>
          ...
          <p style={{ margin: "0 0 10px" }}>
            <strong>To add existing customers:</strong> Click <strong>"+ Enroll Customer"</strong> above.
          </p>
          <p style={{ margin: 0, color: "#6b7280" }}>Going forward, all new paid orders will automatically earn points via webhook.</p>
        </div>
      </s-section>
    </s-page>
  );
